...
import { Heart, Share2, MoreHorizontal, Check, Loader2, ExternalLink, X, Copy, CheckCheck, LayoutTemplate } from 'lucide-react';
...
// ==========================================
// NEW: OpenGraph Preview Panel
// ==========================================
function OpenGraphPreviewPanel({ pin, onClose }: { pin: any; onClose: () => void }) {
  ...
  useEffect(() => {
    const generatePreview = async () => {
      try {
        const metadata = pin.metadata && typeof pin.metadata === 'object' && Object.keys(pin.metadata).length > 0 ? pin.metadata : null;
        let platform = 'inspowall';
        let platformId = pin.rawImage;

        if (metadata) {
          if (metadata.src?.original?.includes('pexels.com') || metadata.photographer) {
            platform = 'pexels';
            platformId = String(metadata.id || pin.rawImage);
          } else if (metadata.alternative_slugs || metadata.urls?.raw?.includes('unsplash.com')) {
            platform = 'unsplash';
            platformId = String(metadata.id || pin.rawImage);
          }
        }
        ...
        if (res && res.success && res.url) {
          ...
          throw new Error(res.error || 'Failed to generate OpenGraph image');
        }
      } catch (err: any) {
  ...
  return (
    <div className="absolute inset-0 z-40 bg-surface/95 backdrop-blur-md rounded-3xl p-8 flex flex-col items-center justify-center">
      <button
        onClick={onClose}
      ...
      <h3 className="text-xl font-display font-bold text-ink-invert mb-6 flex items-center gap-2">
        <LayoutTemplate className="text-neon" /> Social Card Preview
      </h3>

      <div className="w-full max-w-lg aspect-[1200/630] bg-[#0d1117] rounded-2xl border border-black/10 dark:border-white/10 overflow-hidden relative shadow-2xl flex items-center justify-center">
        {loading && (
          <div className="flex flex-col items-center gap-2 text-white">
            <Loader2 size={32} className="animate-spin text-neon" />
            <span className="text-xs font-mono opacity-70">Rendering Edge Card...</span>
          </div>
        )}
        {error && (
        ...
        {ogUrl && !loading && (
          ...
          <img src={ogUrl} alt="OpenGraph Preview" className="w-full h-full object-cover" />
        )}
      </div>

      <div className="mt-8 w-full max-w-lg space-y-3">
        <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block">Edge Public URL</label>
        <div className="flex gap-2">
          <input
            readOnly
            value={ogUrl || 'Generating...'}
            className="flex-1 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl px-4 py-2.5 text-xs font-mono text-ink-invert outline-none"
          />
          <button
            onClick={handleCopy}
            disabled={!ogUrl}
            className="bg-neon text-ink font-bold px-4 py-2.5 rounded-xl hover:opacity-90 transition-all flex items-center gap-2 disabled:opacity-50"
          >
...
function SourceMetadataPanel({ metadata, onClose }: { metadata: any; onClose: () => void }) {
  ...
  return (
    <div className="absolute inset-0 z-30 bg-surface/95 backdrop-blur-sm rounded-3xl p-8 flex flex-col overflow-y-auto">
      <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 rounded-full bg-black/5 dark:bg-white/10 flex items-center justify-center hover:bg-black/10 dark:hover:bg-white/20 transition-colors text-ink-invert">
        <X size={16} />
      </button>
      ...
      <div className="flex items-center gap-4 mb-6 pb-6 border-b border-black/10 dark:border-white/10">
        ...
        <div className="min-w-0">
          <a href={credit.profileUrl} target="_blank" rel="noreferrer noopener" className="font-bold text-ink-invert hover:text-neon transition-colors block truncate">{credit.name}</a>
      ...
      <div className="flex items-center gap-2 mb-6">
        ...
        <span className="text-sm text-gray-500">
          Photo from <a href={source === 'pexels' ? 'https://www.pexels.com' : 'https://unsplash.com'} target="_blank" rel="noreferrer noopener" className="text-neon hover:underline">{credit.sourceName}</a>
        </span>
      </div>
      ...
      <div className="flex flex-col gap-2 mt-auto">
        {links.map((link) => (
          <a key={link.url} href={link.url} target="_blank" rel="noreferrer noopener" className="flex items-center justify-between gap-3 px-4 py-3 rounded-xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 transition-colors text-sm font-medium text-ink-invert border border-black/5 dark:border-white/5">
...
function SharePanel({ pin, onClose }: { pin: any; onClose: () => void }) {
  ...
  const shareTargets = [
    ...
    { label: 'Share on Pinterest', icon: <svg viewBox="0 0 24 24" className="w-[18px] h-[18px] fill-current text-red-500"><path d="M12 0C5.373 0 0 5.373 0 12c0 5.084 3.163 9.426 7.627 11.174-.105-.949-.2-2.405.042-3.441.218-.937 1.407-5.965 1.407-5.965s-.359-.719-.359-1.782c0-1.668.967-2.914 2.171-2.914 1.023 0 1.518.769 1.518 1.69 0 1.029-.655 2.568-.994 3.995-.283 1.194.599 2.169 1.777 2.169 2.133 0 3.772-2.249 3.772-5.495 0-2.873-2.064-4.882-5.012-4.882-3.414 0-5.418 2.561-5.418 5.207 0 1.031.397 2.138.893 2.738a.36.36 0 0 1 .083.345l-.333 1.36c-.053.22-.174.267-.402.161-1.499-.698-2.436-2.889-2.436-4.649 0-3.785 2.75-7.262 7.929-7.262 4.163 0 7.398 2.967 7.398 6.931 0 4.136-2.607 7.464-6.227 7.464-1.216 0-2.359-.632-2.75-1.378l-.748 2.853c-.271 1.043-1.002 2.35-1.492 3.146C9.57 23.812 10.763 24 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0z" /></svg>, href: `https://pinterest.com/pin/create/button/?url=${encodeURIComponent(url)}&description=${encodeURIComponent(pin.title)}`, onClick: null },
    ...
    <div className="absolute inset-0 z-30 bg-surface/95 backdrop-blur-sm rounded-3xl p-8 flex flex-col overflow-y-auto">
      ...
      <div className="flex flex-col gap-2">
        {shareTargets.map((target) =>
          target.href ? (
            <a key={target.label} href={target.href} target="_blank" rel="noreferrer noopener" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 transition-colors text-sm font-medium text-ink-invert border border-black/5 dark:border-white/5">
              <span className="flex-shrink-0 text-ink-invert">{target.icon}</span>{target.label}
            </a>
          ) : (
            <button key={target.label} onClick={target.onClick!} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 transition-colors text-sm font-medium text-ink-invert border border-black/5 dark:border-white/5 text-left">
              <span className="flex-shrink-0 text-ink-invert">{target.icon}</span>{copied && target.label === 'Copy link' ? 'Copied!' : target.label}
            </button>
          )
        )}
      </div>
...
export function PinDetailsSection({
  ...
  return (
    <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col relative">
      ...
      {showOgPreview && <OpenGraphPreviewPanel pin={pin} onClose={closeAll} />}
      ...
      <div className="flex justify-between items-start mb-8">
        <div className="flex gap-3">
          ...
          {/* NEW: OpenGraph Preview Button */}
          <button
            onClick={() => { closeAll(); setShowOgPreview(true); }}
            className="w-12 h-12 rounded-full bg-black/5 dark:bg-white/5 flex items-center justify-center hover:bg-black/10 dark:hover:bg-white/10 transition-colors text-gray-500 dark:text-gray-300 hover:text-ink-invert"
        ...
        >
          {isSaving ? <Loader2 size={20} className="animate-spin" /> : isSaved ? <><Check size={20} /><span>Saved</span></> : <span>Save</span>}
        </button>
      </div>

      <h1 className="text-4xl md:text-5xl font-display font-bold mb-2 tracking-tight text-ink-invert">{pin.title}</h1>
      <div className="flex items-center gap-4 text-sm text-gray-500 mb-6">
        <span className="flex items-center gap-1.5"><Heart size={14} className={isLiked ? 'text-red-500 fill-red-500' : ''} />{likesCount} likes</span>
        <span>•</span>
      ...
      <div className="flex items-center justify-between mt-auto pt-8 border-t border-black/10 dark:border-white/10">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            {metadata && source && (() => {
              const credit = getSourceCredit(metadata, source);
              return <a href={credit.profileUrl} target="_blank" rel="noreferrer noopener" className="text-xs text-gray-400 hover:text-neon transition-colors mt-0.5 block">{credit.handle}@{credit.sourceName.toLowerCase()}</a>;
            })()}
          </div>
        </div>

        <button onClick={() => setFollowing((f) => !f)} className={`px-6 py-2.5 rounded-full font-medium transition-all border ${following ? 'bg-neon text-ink border-neon shadow-[0_0_12px_rgba(204,255,0,0.3)]' : 'bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/20 text-ink-invert border-black/10 dark:border-transparent'}`}>
