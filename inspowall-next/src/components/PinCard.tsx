...
export function PinCard({ id, image, title, author, height, initiallySaved = false, initiallyLiked = false, likes_count = 0 }: any) {
  ...
  return (
    <motion.div
      className="relative break-inside-avoid mb-3 md:mb-4 group cursor-zoom-in"
    ...
    >
      <Link href={`/pin/${id}`} className="block relative rounded-2xl overflow-hidden bg-surface shadow-md">
        {/* Aspect-ratio driven wrapper — 300 is the base width used when height was
            originally calculated in CreatePin (aspectRatio * 300), so this reconstructs
            correct proportions at any column width. next/image fills this box. */}
        <div className="relative w-full" style={{ aspectRatio: `300 / ${height}` }}>
          <Image
            src={image}
            alt={title}
            fill
        ...
        <div
          className={`absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent transition-opacity duration-300 flex flex-col justify-between p-4 z-10 ${
      ...
      <div className="mt-3 px-1 flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          <h3 className="font-medium text-sm text-ink-invert line-clamp-2 leading-snug break-words">{title}</h3>
        ...
        >
          {isLiking ? <Loader2 size={14} className="animate-spin" /> : <Heart size={15} fill={isLiked ? 'currentColor' : 'none'} />}
