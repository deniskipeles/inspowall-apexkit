...
const CATEGORIES = ['Architecture', 'Design', 'Art', 'Technology', 'Nature', 'Fashion', 'Photography', 'Food'];
...
export function CreatePinForm() {
  ...
  return (
    <div className="max-w-5xl mx-auto py-8">
      <button onClick={() => router.back()} className="flex items-center gap-2 text-gray-400 hover:text-ink-invert transition-colors mb-8 group">
            ...
            <AnimatePresence mode="wait">
              {previewUrl ? (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0">
                  {/* Local blob preview of the just-selected file — next/image supports
                      blob: URLs only when unoptimized, which is appropriate here since
                      there's nothing to optimize about a transient client-side object URL. */}
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              >
                {CATEGORIES.map((cat) => (
                  <option key={cat} value={cat} className="bg-surface text-ink-invert">
                    {cat}
                  </option>
                ))}
              </select>
